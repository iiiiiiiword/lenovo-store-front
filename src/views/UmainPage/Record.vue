<template>
  <body>
    ...
    <div style="height: 50px;font-size: 28px;text-align: center">
      购买记录
    </div>
    <section>
      <el-table
        ...
        <el-table-column label="操作" width="290px">
          <template slot-scope="scope">
            ...
            <el-button
              type="primary"
              size="mini"
              @click="SubmitJudge(scope.$index, scope.row)">评价
            </el-button>
            <el-button
              type="success"
              size="mini"
              icon="el-icon-platform-eleme"
              @click="AfterBuy(scope.$index, scope.row)">售后服务
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <Propose></Propose>
    <Down></Down>
    <Judge :JudgeDiaVisible.sync="JudgeDiaVisible"
            :idToJudge="idToJudge">
    </Judge>
    <AfterBuy :AfterBuyDiaVisible.sync="AfterBuyDiaVisible"
            :idToJudge="idToJudge">
    </AfterBuy>
  </body>
</template>
...
import AfterBuy from "@/component/AfterBuy";
...
export default {
  ...
  data(){
    return{
      ...
      JudgeDiaVisible:false,
      idToJudge:-1,
      AfterBuyDiaVisible:false
    }
  },
  methods:{
    doSearch(){
    ...
    loadTable () {
      this.searchFormData.searchWord = this.idToUpdate
      this.$appJsonPost({
        url: '/buy/selectPage',
    ...
    SubmitJudge(index , row){
      this.JudgeDiaVisible = true
      this.idToJudge = row.id
    },
    AfterBuy(index , row){
      this.AfterBuyDiaVisible = true;
      this.idToJudge = row.computerId
    },
    watchScroll() {
      var scrollTop =
        window.pageYOffset ||
